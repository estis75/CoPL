...
impl EvalML3 {
  ...
  pub fn get_tree_eapp(&self) -> (Option<RuleTree>, bool) {
    ...
    if let Some(cap) = self.get_regex(OBJ).captures_iter(&self.obj).next() {
      let val = format!("{} |- {} evalto ?", &cap[1].trim(), &cap[2]);
            ...
            if let Some(c) = c {
              ...
              if let Some(tmpc) = Regex::new(r".* evalto (.*)").unwrap().captures_iter(&c.val).next() {
                ...
                if e2.is_ok() {
                  ...
                  let val = format!("{} |- {} evalto {}", self.format_vectored_env(vec), &e1.2, &cap[4]);
                  dbg!(&val);
                  let c = EvalML3{obj: val}.solver();
                  if let Some(c) = c {
                    tp.push(c.clone());
                    if let Some(tmpc) = Regex::new(r".* evalto (.*)").unwrap().captures_iter(&c.val).next() {
                      if &self.unwrap_if_parened(cap[4].to_string()) == "?" {
                        let val = format!("{} |- {} {} evalto {}", &cap[1].trim(), &cap[2].trim(), cap[3].trim().to_string(), &tmpc[1]);
                        v = Some(RuleTree{
                          obj: OBJ,
                          val,
                          node: Some(tp)
                        ...
                        if lhs.len() == rhs.len() 
                         ...
                         && cap[2] == tmpc[2] 
                         && cap[3] == tmpc[3] {
                          let val = format!("{} |- {} {} evalto {}", &cap[1].trim(), cap[2].trim().to_string(), &cap[3].trim(), &cap[4].trim());
                          v = Some(RuleTree{
                            obj: OBJ,
                            val,
                            node: Some(tp)
                      ...
                      }else if self.unwrap_if_parened(cap[4].to_string()) == tmpc[1] {
                        let val = format!("{} |- {} {} evalto {}", &cap[1].trim(), &cap[2].trim(), &cap[3].trim(), &cap[4].trim());
                        v = Some(RuleTree{
                          obj: OBJ,
                          val,
                          node: Some(tp)
