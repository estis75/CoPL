...
impl Display for Value {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Value::List(l) => {
        let ret = l.into_iter().rev().map(|c| format!("{}", c)).fold(String::from("[]"), |lhs, rhs| rhs + " :: " + &lhs);
        write!(f, "{}", ret)
      },
      Value::Variable(s) => write!(f, "{}", s),
      Value::Parentheses(s) => write!(f, "({})", s),
    }
  }
...
impl EvalML4 {
  ...
  pub fn parse_value(&self, val: String) -> Result<Value, String> {
    let parentheses = self.parse_parentheses(&val);
    ...
    let list = self.parse_list(&val.clone());
    if parentheses.is_ok() {
      Ok(Value::Parentheses(Box::new(parentheses.ok().unwrap())))
    }else if intvalue.is_ok() {
      Ok(Value::Int(intvalue.ok().unwrap()))
    }else if boolvalue.is_ok() {
      Ok(Value::Bool(boolvalue.ok().unwrap()))
    }else if list.is_ok() {
      Ok(Value::List(list.ok().unwrap()))
      ...
      Ok(Value::Variable(val))
    }
  }

  pub fn parse_func(&self, val: &str) -> Result<(Vec<(String, Value)>, String, String), ()> {
    let val = val.trim();
    let mut input_iter = val.chars();
    let mut statement = String::new();
    let mut parens = 0;
    if let Some('(') = input_iter.next() {
      parens += 1;
      ...
      ;
      let val = input_iter.collect::<String>();
      let reg = Regex::new(r"^\[fun (.*?) -> (.*)\]$").unwrap();
      let ret = if let Some(cap) = reg.captures_iter(&val).next() {
        Ok((self.get_env(statement.trim().to_string()), cap[1].trim().to_string(), self.unwrap_if_parened(cap[2].to_string())))
      }else{
        Err(())
      };
      ret
  ...
  pub fn parse_recfunc(&self, val: &str) -> Result<(Vec<(String, Value)>, String, String, String), ()> {
    ...
    if let Some('(') = input_iter.next() {
      ...
      let ret = if let Some(cap) = reg.captures_iter(&val).next() {
        Ok((self.get_env(statement.trim().to_string()), cap[1].trim().to_string(), cap[2].trim().to_string(), self.unwrap_if_parened(cap[3].to_string())))
      }else{
        Err(())
      };
      ret
  ...
  pub fn parse_list(&self, val: &str) -> Result<Vec<Value>, ()> {
    let reg = Regex::new(r"(.*)\[\]$").unwrap();
    let ret = if let Some(cap) = reg.captures_iter(val).next() {
      let mut v = Vec::new();
      let mut input_iter = cap[1].chars().rev().peekable();
      while let Some(tpc) = input_iter.next() { if tpc == ':' { break; } }
      input_iter.next();
      while let Some(_) = input_iter.peek()  {
        ...
        let statement = statement.chars().rev().collect::<String>();
        if let Ok(c) = self.parse_value(statement.trim().to_string()) {
          v.push(c);
        }
        input_iter.next();
      }
  ...
  fn parse_parentheses(&self, val: &str) -> Result<Value, ()>  {
    let vlen = val.len();
    let val = val.trim();
    if vlen > 0 && val.chars().nth(0).unwrap() == '(' && val.chars().nth(vlen-1).unwrap() == ')' {
      if let Ok(v) = self.parse_value(val[1..vlen-1].to_string()) {
        Ok(v)
      }else{
        Err(())
      }
  ...
  fn parse_assign(&self, e: &str) -> Result<Option<(String, Value)>, ()>{
    ...
    if lhs == None {
      ...
      }else if let Ok(num) = self.parse_parentheses(&rhs) {
        Ok(Some((lhs, Value::Parentheses(Box::new(num)))))
      }else if let Ok(num) = self.parse_list(&rhs) {
        Ok(Some((lhs, Value::List(num))))
  ...
  pub fn get_tree_eint(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::EInt)).captures_iter(&self.obj).next() {
  ...
  pub fn get_tree_ebool(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::EBool)).captures_iter(&self.obj).next() {
      ...
      for (x, val) in env.iter() {
        if x.trim() == cap[2].trim() {
          if cap[3].trim() == "?" {
            ...
            let valr = self.parse_value(cap[3].trim().to_string());
            if valr.is_ok() && &valr.ok().unwrap() == val {
              v = Some(RuleTree{
                obj: OBJ,
                val: format!("{} |- {} evalto {}", self.format_vectored_env(&env), x, val),
                node: None
  ...
  pub fn get_tree_eift(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::EIfT)).captures_iter(&self.obj).next() {
  ...
  pub fn get_tree_eiff(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::EIfF)).captures_iter(&self.obj).next() {
      let mut tp = Vec::with_capacity(3);
  ...
  pub fn get_tree_elet(&self) -> (Option<RuleTree>, bool) {
    const OBJ: Object = Object::EvalML4(Rule::ELet);
    let mut v = None;
    let mut state = false;
    let mut tp = Vec::with_capacity(3);
  ...
  pub fn get_tree_eplus(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::EPlus)).captures_iter(&self.obj).next() {
      let mut tp = Vec::with_capacity(3);
            ...
            if let Some(rhs) = Regex::new(r".* evalto (.*)").unwrap().captures_iter(&rhs.val).next() {
              ...
              if lhs.is_ok() && rhs.is_ok() {
                ...
                if val.is_ok() && lhs + rhs == val.ok().unwrap() {
                  ...
                  let c = EvalML4{obj: tpval}.solver();
                  if let Some(c) = c {
                    tp.push(c);

                    v = Some(RuleTree{
                      obj: Object::EvalML4(Rule::EPlus),
  ...
  pub fn get_tree_eminus(&self) -> (Option<RuleTree>, bool) {
    ...
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::EMinus)).captures_iter(&self.obj).next() {
      let mut tp = Vec::with_capacity(3);
              ...
              if lhs.is_ok() && rhs.is_ok() {
                ...
                if lhs - rhs == val {
                  ...
                  if let Some(c) = c {
                    ...
                    v = Some(RuleTree{
                      obj: Object::EvalML4(Rule::EMinus),
  ...
  pub fn get_tree_etimes(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::ETimes)).captures_iter(&self.obj).next() {
      let mut tp = Vec::with_capacity(3);
              ...
              if lhs.is_ok() && rhs.is_ok() {
                ...
                if lhs * rhs == val {
                  ...
                  if let Some(c) = c {
                    ...
                    v = Some(RuleTree{
                      obj: Object::EvalML4(Rule::ETimes),
                      val: format!("{} |- {} * {} evalto {}", &cap[1].trim(), &cap[2], &cap[3], val),
                      node: Some(tp)
                    });
                    state = true;
  ...
  pub fn get_tree_elt(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::ELt)).captures_iter(&self.obj).next() {
      let mut tp = Vec::with_capacity(3);
  ...
  pub fn get_tree_bplus(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::BPlus)).captures_iter(&self.obj).next() {
  ...
  pub fn get_tree_bminus(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::BMinus)).captures_iter(&self.obj).next() {
      let i1 = self.parse_int(cap[1].to_string());
      let i2 = self.parse_int(cap[2].to_string());
      let i3 = cap[3].to_string();
      if i1.is_ok() && i2.is_ok() {
        ...
        if i3.is_ok(){
          ...
          if i1 - i2 == i3 {
            v = Some(RuleTree{
              obj: Object::EvalML4(Rule::BMinus),
  ...
  pub fn get_tree_btimes(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::BTimes)).captures_iter(&self.obj).next() {
      let i1 = self.parse_int(cap[1].to_string());
      let i2 = self.parse_int(cap[2].to_string());
      let i3 = cap[3].to_string();
      if i1.is_ok() && i2.is_ok() {
        ...
        if i3.is_ok(){
          ...
          if i1 * i2 == i3 {
            v = Some(RuleTree{
              obj: Object::EvalML4(Rule::BTimes),
  ...
  pub fn get_tree_blt(&self) -> (Option<RuleTree>, bool) {
    let mut v = None;
    let mut state = false;
    if let Some(cap) = self.get_regex(Object::EvalML4(Rule::BLt)).captures_iter(&self.obj).next() {
      let i1 = self.parse_int(cap[1].to_string());
      let i2 = self.parse_int(cap[2].to_string());
      let i3 = cap[3].to_string();
      if i1.is_ok() && i2.is_ok() {
