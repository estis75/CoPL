...
impl EvalML4 {
  fn token_parser(input: &str) -> Result<Vec<String>, ()> {
    ...
    while let Some(_) = input_iter.peek() {
      ...
      if token == "+" || token == "-" || token == "*" || token == "<" || parens != 0 {
        ...
        ret.clear();
        break;
      }else if token == "match" || token == "with" {
        ret.clear();
        break;
      }else{
  ...
  pub fn get_tree_efun(&self) -> (Option<RuleTree>, bool) {
    const OBJ: Object = Object::EvalML4(Rule::EFun);
    let mut v = None;
    let mut state = false;
  ...
  pub fn get_tree_eapp(&self) -> (Option<RuleTree>, bool) {
    const OBJ: Object = Object::EvalML4(Rule::EApp);
    let mut v = None;
    let mut state = false;
  ...
  pub fn get_tree_eletrec(&self) -> (Option<RuleTree>, bool) {
    const OBJ: Object = Object::EvalML4(Rule::ELetRec);
  ...
  pub fn get_tree_eapprec(&self) -> (Option<RuleTree>, bool) {
    const OBJ: Object = Object::EvalML4(Rule::EAppRec);
